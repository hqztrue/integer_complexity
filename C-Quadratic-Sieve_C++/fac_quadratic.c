...
void register_relation_kind_1(qs_sheet * qs, const cint * KEY, const qs_sm * const  args[4]) {
	...
	for (i = 0; i < 4;) {
		...
		const qs_sm *  idx = args[i++], *  const end_index = args[i++];
...
void register_relation_kind_2(qs_sheet * qs, const cint * KEY, const cint * VALUE, const qs_sm * const  args[4]) {
	...
	if (BEZOUT) {
		...
		simple_dup_cint(new1->X + 1, BEZOUT, &qs->mem.now);
		// The 2nd new1comer become the root of the linked list.
		new1->axis.next = old, node->value = new1 = old = new1;
	} else {
		// All but the 2nd have no special treatment.
		qs->mem.now = new1->X + 1; // they come at the end of the linked list.
		simple_dup_cint(new1->X, KEY, &qs->mem.now);
		if (old) {
	...
	new1->Y.length += (qs_sm) (args[3] - args[2]);
	qs->mem.now = new1->Y.data + new1->Y.length;

	if (old) {
		BEZOUT = old->X + 1 ; // the modular inverse was stored here.
		cint_mul_mod(qs->calc, new1->X, BEZOUT, &qs->constants.kN, &qs->vars.CYCLE);
		do {
			if (old != new1) {
				...
				const qs_sm* args[4]={ begin, pen, 0, 0, };
...
void finalization_part_1(qs_sheet * qs, const uint64_t *  const lanczos_answer) {
