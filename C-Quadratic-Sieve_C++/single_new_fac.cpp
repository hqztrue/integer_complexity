...
const vector<pair<ull,int>> prime_factors1(ull x){
...
const vector<pair<ull,int>> prime_factors(ull x){
	...
	static char str[105];
	sprintf(str,"%I64d",x);
	const int bits = 64 + 4 * (int) strlen(str);
	cint_init_by_string(&N, bits, str, 10); // init the number as a cint.
...
uchar dfs(ull x,int t=10){  //computes f(x) for a single x, overall using <=t 1's for addition.
	if (x<=n0)return a[x];
	auto it=H.find(x);
	if (it!=H.end()&&it->second.first>=t)return it->second.second;
	uchar ans=inf;
	for (int i=1;i<=g[t]&&i<x;++i)
		if (a[i]<=t)ans=min(ans,uchar(dfs(x-i,t-a[i])+a[i]));
...
int main()
{
	...
	for (auto mul:primes){
		...
		for (int i=1;;++i,x*=mul){
			...
			n=x;
			//printf("i=%d n=%I64d\n",i,n);
			int v=i*a[mul],d=defect(n,v-1);
			//printf("def=%d\n",d);
			int ans=dfs(n,3);
			//printf("time=%d\n",clock()-t1);
			//printf("size=%d\n",H.size());
			if (ans<v){
				printf("err: %d^%d=%I64d ans=%d tgt=%d\n",mul,i,n,ans,v);
				//exit(0);
			}
			if (x>N0/mul)break;
		}
