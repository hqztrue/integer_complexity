...
template<class T>
const vector<pair<T,uchar>> prime_factors128(T x){
...
const int D=5;
void dfs_print(u128 x,int v){
	if (x<=n0){
		cout<<s[x];
		return;
	}
	for (int i=1;i<=g[D]&&i<x;++i)
		if (a[i]<=D){
			ushort v1=dfs128(x-i,0),v2=a[i];
			if (v1+v2==v){
				dfs_print(x-i,v1);
				cout<<"+";
				dfs_print(i,v2);
	...
	for (auto &g:get_factors(x))
		if (g>1&&g<=x/g){
			ushort v1=dfs128(g,0),v2=dfs128(x/g,0);
			if (v1+v2==v){
				cout<<"(";
				dfs_print(x/g,v2);
				cout<<")*(";
...
int main()
{
	...
	printf("time=%d\n",clock()-t1);
	u128 N0=1; N0<<=120;
	//print_expr(556668616404130ull);
	//print_expr(3652302792226978690ull);
	
	//printf("def=%d\n",defect(4265932874987493736,127));
	print_expr(4265932874987493736.Q);
	print_expr(12285886679963981959681.Q);
	print_expr(230752268637185668771.Q);
	print_expr(236736367459211723401.Q);
	exit(0); 
	...
	//vector<int> primes={577,811,109,433,163,487};
	vector<int> primes={2,163,487};  //433
	//vector<int> primes={2};
	for (auto mul:primes){
		printf("mul=%I64d\n",mul);
		u128 x=mul;
